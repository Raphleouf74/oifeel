...
// ======================
// ======================
// STATISTIQUES (compteurs anonymes — voir services/analytics.cjs)
// ======================
const FUNNEL_ROWS = [
    ["visit", "Visites (sessions)"],
    ["guest_login", "Connexions en invité"],
    ["register", "Inscriptions"],
    ["login", "Connexions"],
    ["post_created", "Posts publiés"],
    ["story_created", "Stories publiées"],
    ["share_open", "Feuille de partage ouverte"],
    ["share_native", "Partages via le système (Insta, WhatsApp…)"],
    ["share_download", "Images enregistrées"],
    ["share_link", "Liens copiés"],
    ["pwa_installed", "Installations de l'app"],
    ["push_prompt_shown", "Demandes de notifications"],
    ["push_enabled", "Notifications activées"],
    ["push_denied", "Notifications refusées"],
];
...
async function loadAnalytics() {
    ...
    el.innerHTML = `<div class="loading-state"><div class="spinner"></div><p>Chargement des statistiques...</p></div>`;

    try {
        const res = await fetch(`${API}/admin/analytics?days=${days}`, { headers: adminHeaders() });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const { days: rows } = await res.json();

        const total = {};
        rows.forEach((r) => {
            Object.entries(r.events || {}).forEach(([k, v]) => { total[k] = (total[k] || 0) + v; });
        });
        const visits = total.visit || 0;
        const pct = (n) => (visits ? ((n / visits) * 100).toFixed(1) + " %" : "—");
        const fmt = (n) => (n || 0).toLocaleString("fr-FR");

        const funnel = FUNNEL_ROWS.map(([k, label]) => `
            <tr><td>${label}</td><td style="text-align:right">${fmt(total[k])}</td>
            <td style="text-align:right;opacity:.7">${k === "visit" ? "" : pct(total[k] || 0)}</td></tr>`).join("");

        const sources = Object.entries(total)
            .filter(([k]) => k.startsWith("src_"))
            .sort((a, b) => b[1] - a[1])
            .map(([k, v]) => `<tr><td>${k.slice(4)}</td><td style="text-align:right">${fmt(v)}</td>
                <td style="text-align:right;opacity:.7">${pct(v)}</td></tr>`).join("")
            || `<tr><td colspan="3" style="opacity:.6">Pas encore de données.</td></tr>`;

        const daily = rows.slice().reverse().map((r) => {
            const e = r.events || {};
            return `<tr><td>${r.date}</td><td style="text-align:right">${fmt(e.visit)}</td>
                <td style="text-align:right">${fmt((e.guest_login || 0) + (e.register || 0))}</td>
                <td style="text-align:right">${fmt(e.post_created)}</td>
                <td style="text-align:right">${fmt(e.share_open)}</td></tr>`;
        }).join("") || `<tr><td colspan="5" style="opacity:.6">Pas encore de données.</td></tr>`;

        el.innerHTML = `
            <div class="stat-grid">
                <div class="stat-card"><div class="stat-label">Visites</div><div class="stat-value accent">${fmt(visits)}</div></div>
                <div class="stat-card"><div class="stat-label">Posts publiés</div><div class="stat-value success">${fmt(total.post_created)}</div></div>
                <div class="stat-card"><div class="stat-label">Visites → post</div><div class="stat-value warn">${pct(total.post_created || 0)}</div></div>
                <div class="stat-card"><div class="stat-label">Visites via un lien partagé</div><div class="stat-value">${fmt(total.src_share)}</div></div>
            </div>
            <div class="table-wrapper" style="margin-bottom: 18px">
                <div class="table-header"><div class="table-title">Parcours (${days} derniers jours)</div></div>
                <table><thead><tr><th>Étape</th><th style="text-align:right">Total</th><th style="text-align:right">% des visites</th></tr></thead>
                <tbody>${funnel}</tbody></table>
            </div>
            <div class="table-wrapper" style="margin-bottom: 18px">
                <div class="table-header"><div class="table-title">D'où viennent les visites</div></div>
                <table><thead><tr><th>Canal</th><th style="text-align:right">Visites</th><th style="text-align:right">Part</th></tr></thead>
                <tbody>${sources}</tbody></table>
                <p style="font-size:12px;opacity:.6;padding:10px 16px 14px">
                    Astuce : mets <code>?utm_source=tiktok</code> (ou instagram, discord, reddit…) au bout du lien de ta bio pour savoir quel canal fonctionne.
                </p>
            </div>
            <div class="table-wrapper">
                <div class="table-header"><div class="table-title">Jour par jour</div></div>
                <table><thead><tr><th>Date</th><th style="text-align:right">Visites</th><th style="text-align:right">Invités + inscrits</th><th style="text-align:right">Posts</th><th style="text-align:right">Partages</th></tr></thead>
                <tbody>${daily}</tbody></table>
            </div>`;
    } catch (err) {
        console.error("Analytics:", err);
        el.innerHTML = `<div class="loading-state"><p>Impossible de charger les statistiques (${err.message}).</p></div>`;
    }
}
